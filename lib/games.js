...
Game.prototype.season = function() {
    var season = this.date.getFullYear();
...
Game.prototype.weeknumber = function() {
    var parsed = parseInt(this.week, 10);
    if (isFinite(parsed)) {
        return parsed;
    } else if (this.week == 'WildCard') {
        return 18;
    } else if (this.week == 'Division') {
        return 19;
    } else if (this.week == 'ConfChamp') {
        return 20;
    } else if (this.week == 'SuperBowl') {
        return 21;
    }
    throw new Error('week could not be parsed correclty');
};
