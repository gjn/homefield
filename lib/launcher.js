...
var Launcher = function () {
    ...
    var getFilterFunction = function(seasonref, weekref) {
        return function(game) {
            ...
            //Previous season
                ...
                //we exclude postseason games from previous seasons
                if (gameweek <= 17 &&
                    gameweek > weekref) {
                    return true;
                }
    ...
    var analyse = function() {
        ...
        var enoughGames = function(gas) {
            var weekOfLast = gas[gas.length - 1].weeknumber();
            var weekOfFirst = gas[0].weeknumber();
            if (weekOfLast < weekOfFirst) {
              weekOfFirst -= 17;
            }
            if (weekOfLast - weekOfFirst >= 16) {
              return true;
            }
        ...
        if (type == 'all') {
            for (i = 0; i < gs.length; i++) {
                ...
                if (lastweek != refGame.week) {
                    analyseGames = Games.filter(games.games, getFilterFunction(refGame.season(), refGame.weeknumber()));
                    if (enoughGames(analyseGames)) {
                        getNewAnalyser().analyse(analyseGames);
                    }
