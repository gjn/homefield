...
JSONExporter.prototype.add = function(results) {
    ...
    var weekGames = optimizeGamesForExport(Games.filter(results.games, weekGamesFilter(season, week)));
    ...
    newRes.SOS = {};
    _.each(results.SOS, function(S, key) {
      newRes.SOS[key] = parseFloat(S.avg.toFixed(1));
    });
    newRes.SOSPlayed =  {};
    _.each(results.SOSPlayed, function(S, key) {
      newRes.SOSPlayed[key] = parseFloat(S.avg.toFixed(1));
      /*
      newRes.SOSPlayed[key] = {
        sum: parseFloat(S.sum.toFixed(1)),
        num: parseInt(S.num)
