...
ExponentialMovingAverageAnalyser.prototype.analyse = function(games) {
    ...
    var lastGame = this.games[this.games.length - 1];
    var season = lastGame.season();

    var filenameBase = '.artefacts/res/' + season + '-' + lastGame.week;

    //this.exportCSV(filenameBase + '.csv');
    this.exportJSON(filenameBase + '.js');
};
...
ExponentialMovingAverageAnalyser.prototype.analyseGame = function(game) {
    ...
    var calculate = function(homestat, awaystat, ht, at, havgs, aavgs, alpha) {
        var htNew = getNewValues(homestat, awaystat, at.stat, havgs);
        var atNew = getNewValues(awaystat, homestat, ht.stat, aavgs);
    ...
    calculate(game.homestat, game.awaystat, hstat, astat, this.averages.all, this.averages.all, this.alpha);
    ...
    calculate(game.homestat, game.awaystat, hstat, astat, this.averages.home, this.averages.away, this.alpha);
...
ExponentialMovingAverageAnalyser.prototype.exportJSON = function(filename) {
    'use strict';
    var that = this;
    ...
    var gamesBackup = this.games;
    var teamstatsBackup = this.teamstats;
    //We create arrays for better handling in d3
    var results = {};
    _.each(this.teamstats, function(ts, key) {
      results[key] = {};
      _.each(ts, function(res) {
        results[key][res.team] = optimizeForExport(res);
      });
    });
    this.results = results;
    this.games = undefined;
    this.teamstats = undefined;


    var stream = fs.createWriteStream(filename);
    //To get rid of quotes from keys
    var string = stringify(this, null, 2);
    var newObj = JSON.parse(string);
    stream.write(util.inspect(newObj, {depth: null}));

    //restore
    this.games = gamesBackup;
    this.teamstats = teamstatsBackup;
