...
ExponentialMovingAverageAnalyser.prototype.analyse = function(games) {
    ...
    var filenameBase = '.artefacts/results/results-' + season + '-' + lastGame.week;

    this.exportCSV(filenameBase + '.csv');
    this.exportJSON(filenameBase + '.json');
...
ExponentialMovingAverageAnalyser.prototype.exportJSON = function(filename) {
    ...
    var stream = fs.createWriteStream(filename);
    //Stringify will create string with keys in quotes.
    //We don't want that. But we also can't use the
    //util.inspect directly, as this will include 'undefined'
    //keys as well. So we stringify (to get rid of 'undefined'
    //keys, read the string back into an object and
    //util.inspect this new object
    //Et Voilà, we'll have the results how we want them
    var stringg = stringify(this, null, 2);
    var newObj = JSON.parse(stringg);
